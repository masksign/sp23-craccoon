...
//  This is the reference code matching the order of operations in Python.

//  redefine names locally
#define racc_keygen racc_keygen_ref
#define racc_sign   racc_sign_ref
#define racc_verify racc_verify_ref
...
//  this flag puts raw NTT-domain values into the computation.
//  this will change the test values
//#define NTT_RAW
...
void racc_keygen(racc_pk_t *pk, racc_sk_t *sk)
{
    ...
    int64_t mt[RACC_K][RACC_D][RACC_N];
    int64_t mt2[RACC_K][2 * RACC_D][RACC_N];
    ...
    for (i = 0; i < RACC_K; i++) {
        for (j = 0; j < RACC_D; j++) {
            polyr_cmul(mt[i][j], sk->s[0][j], ma[i][0]);
            for (k = 1; k < RACC_ELL; k++) {
                polyr_mula(mt[i][j], sk->s[k][j], ma[i][k], mt[i][j]);
            }
            polyr_intt(mt[i][j]);
        }
    }
    ...
    for (i = 0; i < RACC_K; i++) {
        for (j = 0; j < 2 * RACC_D; j++) {
            polyr_round(mt2[i][j], mt2[i][j], RACC_LPT, RACC_CTT, RACC_QT);
        }
    }
    ...
    for (i = 0; i < RACC_K; i++) {
        racc_decode(pk->t[i], mt2[i], 2 * RACC_D, RACC_QT);
    }
                ...
                size_t mlen, racc_sk_t *sk)
{
    ...
    int64_t mz[RACC_ELL][RACC_D][RACC_N];
    ...
    do {
        ...
        for (i = 0; i < RACC_ELL; i++) {
            racc_decode(sig->z[i], mz[i], RACC_D, RACC_Q);
        }
